...
4. The unified factorial dataset is in `data/canonical/r{3,4,5,6}_*.jsonl`. Per-battery raw and graded files (`data/{category}/`) and the chronological per-round provenance (`data/archive_chronological/round{n}/`) are also deposited.
5. Each deposited experiment category below has its own subdirectory under `data/` with raw outputs, three-judge consensus grades, and a per-experiment README.
...
| Temperature sweep | 320 | 4 cells × 6 models × 3 temperatures (320 valid of 360 dispatched) |
| B11 deployment-realistic, frontier | 12,417 | 35 cells × 9 models × 2 conditions × N=20 (of 12,600 dispatched) |
| B12 deployment-realistic, deployment-tier | 8,220 | 35 cells × 6 models × 2 conditions × N=20 (of 8,400 dispatched) |
| **Grand total (three-judge graded)** | **36,177** | every record graded by 3 LLM judges |
| MMLU capability probe | 959 | 80 items × 6 models × 2 conditions, ground-truth graded (not LLM-judged) |
| **Grand total (all graded trials)** | **37,136** | as reported in the manuscript |
...
**Directory**: `data/canonical/r3_cross_cultural_matrix.jsonl` (unified; carries verbatim `response_text` and three-judge grades on every record). Per-round raw provenance under `data/archive_chronological/round{1,2,3}/`.
...
**Used in paper**: Figures 1, 3, 4 and Extended Data Figure 1; main §§1-2.


### 2. Mechanism — priority instruction
**Question**: Does the safety-vs-context-following tension drive the phenomenon causally?
**Directory**: `data/canonical/r4_priority_instruction.jsonl` (unified). Per-round raw provenance under `data/archive_chronological/round4/`.
...
**Used in paper**: Figure 5; main text (priority-instruction section); SN 10.
...
**Models**: 6.
**N**: 1,080 trials.
**Used in paper**: main text (priority-instruction section); SN 24.
...
**Models**: 8.
**N**: 360 trials.
**Used in paper**: SN 21; cited in the main text (priority-instruction section).
...
**Directory**: `data/canonical/r5_counterfactual.jsonl` (unified). Per-round raw provenance under `data/archive_chronological/round{5,5b}/`.
...
**Used in paper**: Extended Data Figure 2; main text (roleplay section); SN 12.
...
**Directory**: `data/canonical/r6_multiturn.jsonl` (unified). Per-round raw provenance under `data/archive_chronological/round{6,6b}/`.
...
**Used in paper**: Figure 6; main text (roleplay section); SN 11.
...
**N**: 12,180 canonical records, each graded by all three judges (36,540 judge calls).
**Pairwise Cohen's κ**: Opus↔GPT-4o = 0.809, Opus↔Gemini-3-Flash = 0.808, GPT-4o↔Gemini-3-Flash = 0.761.
**Agreement**: 85.8% unanimous, 12.9% 2-of-3 majority, 1.3% all-three-differ.
**Used in paper**: Methods; Supplementary Note 13.

### 8. Negative controls
**Question**: Where does the phenomenon NOT occur?
**Directory**: derived from `data/canonical/r3_cross_cultural_matrix.jsonl` (cells `APARTHEID_SA_LEADER`, `INDIAN_CASTE_MARRIAGE`, `INDIAN_DHARMA_DAUGHTER`, and the `CONTROL_*` no-demonstration probes); no separate folder.
...
**Used in paper**: main §2; Extended Data Figure 1; SN 8.
...
**Used in paper**: main §3 (Lexical specificity is necessary for doctrinal output); Supplementary Note 23.
...
**Used in paper**: Main §6 (Doctrinal compliance under deployment-realistic modern user queries); Figure 2 (six-card verbatim grid); Supplementary Note 27.
...
**Provenance**: Canonical files are derived from the per-round graded files via `scripts/consolidate_canonical.py`. The chronological per-round artefacts are preserved under `data/archive_chronological/round{n}/` so a reviewer can audit the merge.
...
## File-system map (deposit layout, audited 2026-06-11)

```
{deposit_root}/
├── README.md                       # entry point for the GitHub release
...
├── CITATION.cff, LICENSE, requirements.txt, .env.example
├── manuscript/
│   ├── main.md                     # paper main text (+ built main.pdf / main.docx)
│   ├── supplementary.md            # supplementary notes (+ built PDFs)
│   ├── references.bib
│   ├── build.sh                    # pandoc + xelatex build (self-contained: nature.csl + reference.docx vendored here)
│   └── figures/                    # figure sources + rendered PDFs/PNGs
├── prompts/
...
│   ├── temperature_sweep/                   # 320 graded trials + raw + dispatch log
│   ├── mmlu_capability/                     # 959 ground-truth-graded trials + dispatch log
│   ├── b11_pure_modern_probes/              # deployment-realistic frontier (12,417 graded + raw + SUMMARY.md)
│   ├── b12_small_models/                    # deployment-realistic deployment-tier (8,220 graded + raw + SUMMARY.md)
...
│   │   ├── three_judge_kappa.txt            # pre-exclusion April artefact, provenance-annotated
│   │   ├── two_judge_grades.jsonl           # legacy two-judge log, provenance only
│   │   └── kappa_summary.txt                # canonical κ summary (regenerable via compute_kappa.py)
│   ├── archive_chronological/               # raw round-by-round JSONLs, R1-R10 (full provenance)
│   └── _excluded_models_backup_2026-04-29/  # excluded OpenAI models (gpt-5, gpt-5-mini, o3-mini), auditable
└── scripts/                        # all dispatch, grading, analysis, and verification code
    ├── README.md                   # describes every subfolder and top-level script
    ├── dispatch/                   # H-ROUND{1..10}_run.py + new-model dispatch + boundary + B11/B12
    ├── grading/                    # regex graders + three-judge sweeps + compute_kappa.py
    ├── analysis/                   # figure-generating scripts
    ├── verify/                     # number-verification scripts (see verify/README.md)
...
1. Open `MASTER_NUMBERS_DIGEST.md`; every load-bearing rate appears there with its source.
2. Run the three-command verification workflow (documented in `scripts/verify/README.md`):
   `python scripts/verify/verify_sn28_numbers.py` (every deployment-realistic SN 27 number, recomputed from raw),
   `python scripts/compute_master_numbers.py` (regenerates the master numbers from the canonical files),
   `python scripts/grading/compute_kappa.py` (recomputes pairwise κ and the agreement breakdown).
3. For a specific (cell × model) rate, group `data/canonical/r3_cross_cultural_matrix.jsonl` on `consensus_grade` (a five-line snippet is given in `REPRODUCIBILITY.md` §7).

For per-cell × per-model rate tables in SN 4-5, the per-model summary is `data/canonical/per_model_summary.md` and coverage is audited in `data/canonical/coverage_audit.md`.
...
Final grade rule: 2-of-3 majority among three independent LLM judges (Anthropic Opus 4.6, OpenAI GPT-4o, Google Gemini-3-Flash-Preview). If all three differ, fall back to Judge 1 (Opus 4.6). Every record in the canonical, boundary, R9, and R10 corpora is independently graded by all three judges. Pairwise Cohen's κ on the 12,180-record canonical: Opus↔GPT-4o = 0.809, Opus↔Gemini-3-Flash = 0.808, GPT-4o↔Gemini-3-Flash = 0.761. Pairwise κ on the 1,400-record scaled boundary: 0.625 / 0.567 / 0.614 (skewed Grade-D distribution). See `data/methodology_two_judge/` and `prompts/JUDGE_PROMPT.txt`.
...
Replication repository: [https://github.com/FelipeMAffonso/cultural-priming](https://github.com/FelipeMAffonso/cultural-priming) (public, MIT licence).
